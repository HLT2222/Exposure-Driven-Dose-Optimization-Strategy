...
#Compute the Fisher information matrix for each protocol
 ...
 if (nr>1){for(l in 2:nr) sig<-c(sig,model.info$sigmainter[[l]],model.info$sigmaslope[[l]])} 
  ...
  #taking into account previous information
    if (previous.FIM!= ""){
    previous_information<-read.table(file.path(directory,previous.FIM),header=FALSE,sep="")
    #On teste si la previous information matrix a la m?me taille que la matrice actuelle. Si ce n'est pas le cas, on arr?te l'ex?cution et on affiche un message d'erreur : 
...
###                                 Fedorov output                      ###                                                                                                                                                            
###########################################################################
out.fedorov<-function(modelfile="Stdin.r", directory=".", dose, beta, omega, sig.interA,
directory.program=".") {  
#directories  
